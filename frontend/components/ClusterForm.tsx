  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/cluster/analyze`, {
        ...
        body: JSON.stringify({ addresses: addressList, maxDepth: 2 }),
      })
      ...
      setError(err instanceof Error ? err.message : 'Failed to analyze cluster')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="addresses" className="block text-sm font-medium text-gray-400 mb-2">
            Wallet Addresses
          </label>
          <textarea
            id="addresses"
      ...
      {result && (
        <ResultCard title="Cluster Analysis" data={result} />
      )}
    </div>
  )
}
